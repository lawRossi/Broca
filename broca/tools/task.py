...
class TaskManagement(Tool):
    ...
    async def _execute(self, arguments: dict, context: ToolCallContext) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(status=ToolStatus.ERROR, content=f"Error executing action '{action}': {e}")

    def _create_task(self, arguments) -> ToolResult:
        required_fields = ["name", "description"]
        for field in required_fields:
            if field not in arguments:
                return ToolResult(status=ToolStatus.ERROR, content=f"Missing required field: {field}")
        ...
        try:
            ...
            return ToolResult(status=ToolStatus.SUCCESS, content=f"Task created successfully with ID: {task.metadata.id}")
        except ValueError as e:
            return ToolResult(status=ToolStatus.ERROR, content=f"Invalid value: {e}")
        except Exception as e:
            return ToolResult(status=ToolStatus.ERROR, content=f"Error creating task: {str(e)}")

    def _get_task(self, arguments) -> ToolResult:
        task_id = arguments.get("task_id")
        if not task_id:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: task_id")

        task = self.task_manager.get_task(task_id)
        if task:
            return ToolResult(status=ToolStatus.SUCCESS, content=json.dumps(task.model_dump(), indent=2, default=str))
        else:
            return ToolResult(status=ToolStatus.ERROR, content=f"Task with ID {task_id} not found")

    def _get_all_tasks(self) -> ToolResult:
        tasks = self.task_manager.get_all_tasks()
        return ToolResult(status=ToolStatus.SUCCESS, content=json.dumps([task.model_dump() for task in tasks], indent=2, default=str))

    def _get_tasks_by_status(self, arguments) -> ToolResult:
        status = arguments.get("status")
        if not status:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: status")

        try:
            status_enum = TaskStatus(status.lower())
            tasks = self.task_manager.get_tasks_by_status(status_enum)
            return ToolResult(status=ToolStatus.SUCCESS, content=json.dumps(
                [task.model_dump() for task in tasks], indent=2, default=str
            ))
        except ValueError:
            return ToolResult(status=ToolStatus.ERROR, content=f"Invalid status: {status}. Valid values: pending, in_progress, blocked, completed")

    def _get_tasks_by_assignee(self, arguments) -> ToolResult:
        assignee = arguments.get("assignee")
        if not assignee:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: assignee")

        tasks = self.task_manager.get_tasks_by_assignee(assignee)
        return ToolResult(status=ToolStatus.SUCCESS, content=json.dumps([task.model_dump() for task in tasks], indent=2, default=str))

    def _update_task(self, arguments) -> ToolResult:
        task_id = arguments.get("task_id")
        if not task_id:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: task_id")
        ...
        try:
            ...
            if task:
                return ToolResult(status=ToolStatus.SUCCESS, content=f"Task {task_id} updated successfully")
            else:
                return ToolResult(status=ToolStatus.ERROR, content=f"Task with ID {task_id} not found")
        except ValueError as e:
            return ToolResult(status=ToolStatus.ERROR, content=f"Invalid value: {str(e)}")
        except Exception as e:
            return ToolResult(status=ToolStatus.ERROR, content=f"Error updating task: {str(e)}")

    def _delete_task(self, arguments) -> ToolResult:
        task_id = arguments.get("task_id")
        if not task_id:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: task_id")
        ...
        if success:
            return ToolResult(status=ToolStatus.SUCCESS, content=f"Task {task_id} deleted successfully")
        else:
            return ToolResult(status=ToolStatus.ERROR, content=f"Task with ID {task_id} not found")

    def _add_comment(self, arguments) -> ToolResult:
        task_id = arguments.get("task_id")
        ...
        if not task_id:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: task_id")
        if not author:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: author")
        if not content:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: content")
        ...
        if task:
            return ToolResult(status=ToolStatus.SUCCESS, content=f"Comment added to task {task_id} successfully")
        else:
            return ToolResult(status=ToolStatus.ERROR, content=f"Task with ID {task_id} not found")

    def _get_child_tasks(self, arguments) -> ToolResult:
    ...
    def _search_tasks(self, arguments) -> ToolResult:
        query = arguments.get("query")
        if not query:
            return ToolResult(status=ToolStatus.ERROR, content="Missing required field: query")
