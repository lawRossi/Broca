...
const isTextFile = computed(() => {
  ...
  return (
    !previewContent.value.includes('Binary file') &&
...
const cancelEditing = () => {
  if (contentChanged.value) {
    // 如果有未保存的更改，提示用户
    ElMessageBox.confirm('You have unsaved changes. Are you sure you want to cancel?', 'Confirm Cancel', {
...
const handleClosePreview = () => {
  if (isEditing.value && contentChanged.value) {
    ElMessageBox.confirm('You have unsaved changes. Are you sure you want to close?', 'Confirm Close', {
...
const saveFile = async () => {
  try {
    ...
    if (contentSize > maxSize) {
      throw new Error(
        `File content too large (${(contentSize / 1024 / 1024).toFixed(2)}MB > 10MB). Please reduce file size.`
      )
    }

    // 检查文件是否为空
    if (editedContent.value.trim() === '') {
      ElMessageBox.confirm('File content is empty. Save empty file?', 'Confirm Save', {
          ...
          <div class="bg-white rounded-lg border overflow-hidden">
            <table class="w-full">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                >
                  <td class="p-3">
                    <div class="flex items-center gap-2 cursor-pointer" @click="handleFileClick(file)">
                      <el-icon :size="18" :class="file.is_dir ? 'text-blue-500' : 'text-gray-500'">
                        <component :is="getFileIcon(file)" />
                      </el-icon>
                      <span class="font-medium text-gray-900">{{ file.name }}</span>
                      <span v-if="!file.readable" class="text-xs text-red-500" title="Not readable"> 🔒 </span>
                    </div>
                  </td>
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1 cursor-pointer" @click="handleFileClick(file)">
                  ...
                  <div class="text-xs text-gray-600 space-y-1 ml-6">
                    <div class="flex items-center gap-2">
                      <span v-if="!file.is_dir"> Size: {{ getFileSize(file) }} </span>
    ...
    <div class="bg-white border-t border-gray-200 p-2 sm:p-3">
      <div class="flex items-center justify-between text-xs text-gray-600">
        <div>
          ...
          <span v-if="searchQuery" class="text-primary-600"> (filtered) </span>
        </div>
        <div>
