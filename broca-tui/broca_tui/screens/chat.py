...
from broca_tui.widgets.agent_sidebar import AgentSidebar, AbortAgent
...
class ChatScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="chat-screen"):
            ...
            with Horizontal(classes="chat-content", id="chat-content"):
                # Left: Agent sidebar
                yield AgentSidebar(store=self._agent_store, id="agent-sidebar", classes="sidebar")
    ...
    def _setup_connections(self):
        ...
        self._chat_store.on_error(lambda msg: self.notify(msg, severity="error", timeout=5))
        self._chat_store.on_permission_request(lambda d: self.run_worker(self._show_permission_dialog(d)))
        self._chat_store.on_agent_query(lambda d: self.run_worker(self._show_agent_query_dialog(d)))
        self._chat_store.on_message_received(lambda msg: self._on_message_received(msg))
        self._chat_store.on_connection_change(lambda connected: self._on_connection_change(connected))

        # MessageList callbacks
        message_list.set_on_load_more(lambda: self.run_worker(self._load_more_history()))
        message_list.set_on_redo(lambda: self.run_worker(self._chat_store.send_user_message("/redo")))

        # ChatInput callbacks
        chat_input.set_on_send(lambda text, target: self.run_worker(self._send_message(text, target)))

        # Set agents for @mention
        chat_input.set_agents(self._agent_store.agents)

    async def _connect(self):
        """Connect to Socket.IO and load agents."""
        header = self.query_one("#chat-header", ChatHeader)
        message_list = self.query_one("#message-list", MessageList)
    ...
    def _on_message_received(self, msg: Dict[str, Any]):
        ...
        if msg_type == "turn_start":
            self._agent_store.update_agent_status(
                msg.get("agent_id", ""), "running"
            )
        elif msg_type == "turn_end":
            self._agent_store.update_agent_status(
                msg.get("agent_id", ""), "idle"
            )
    ...
    def on_chat_header_navigate_to_sessions(self, event: ChatHeader.NavigateToSessions) -> None:
    ...
    def on_info_sidebar_start_runner(self, event: InfoSidebar.StartRunner) -> None:
        """Handle runner start."""
        self.run_worker(self._chat_store.restart_runner())

    def on_info_sidebar_stop_runner(self, event: InfoSidebar.StopRunner) -> None:
        """Handle runner stop."""
        self.run_worker(self._chat_store.stop_runner())
