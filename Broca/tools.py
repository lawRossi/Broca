...
from Broca.task import TaskStatus, TaskPriority, TaskContext
...
class ExecuteCode(Tool):
    ...
    def _init_tree_sitter(self):
        ...
        try:
            ...
            logger.warning(f"Failed to initialize tree-sitter: {e}. Falling back to regex validation.")
    ...
    def _validate_with_tree_sitter(self, code: str) -> bool:
        ...
        try:
            ...
            dangerous_commands = {
                'rm', 'del', 'rd', 'format', 'shutdown', 'reboot', 'halt',
                'poweroff', 'iptables', 'dd', 'mkfs', 'fdisk', 'sudo', 'su'
            }
                ...
                if node.type == 'command':
                    ...
                    if command_node and command_node.type == 'word':
                        ...
                        if command_name in dangerous_commands:
                            logger.warning(f"Dangerous command detected: {command_name}")
                            return False
                            ...
                            for child in node.children:
                                if child.type == 'word' and child.start_byte < child.end_byte:
                                    ...
                                    if flag in ['-rf', '-r', '-f', '-fr']:
                                        logger.warning(f"Dangerous rm flag detected: {flag}")
                                        return False
                
                # Check for shell injection patterns
                if node.type == 'string' and ('`' in code[node.start_byte:node.end_byte] or
            ...
            logger.warning(f"Tree-sitter validation failed: {e}. Falling back to regex validation.")
            return self._validate_with_regex(code)
