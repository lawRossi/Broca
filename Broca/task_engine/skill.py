...
class FormSkill(Skill):
    ...
    def id_desired_intent(self, slot_mapping, tracker):
        ...
        return (not intents or intent in intents) and intent not in not_intents
            ...
            for slot_mapping in self.get_mappings_for_slot(slot_to_fill):
                if self.id_desired_intent(slot_mapping, tracker):
                    ...
                    if slot_mapping["type"] == self.FROM_ENTITY:
                        value = tracker.get_latest_entity_value(slot_mapping["entity"])
                    ...
                    if value is None and config["prefilled"]:  # try to find a prefilled slot vlaue
    ...
    def validate(self, slot_dict):
        ...
        for slot_name, value in slot_dict.items():
            validate_func = getattr(self, f"validate_{slot_name}")
    ...
    def _perform(self, tracker):
        ...
        for slot_name, _ in self.required_slots:
            if slot_name not in slot_dict:
                next_to_fill_slot = slot_name
                break
        if next_to_fill_slot:
            utter_func = getattr(self, f"utter_ask_for_{next_to_fill_slot}")
            self.utter(utter_func())
        else:
            events.extend(self._submit(tracker))
